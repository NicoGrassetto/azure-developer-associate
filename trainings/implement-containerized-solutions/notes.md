...
Using the familiar `docker build` format, the [az acr build](https://learn.microsoft.com/en-us/cli/azure/acr?view=azure-cli-latest#az-acr-build) command in the Azure CLI takes a context (the set of files to build), sends it to ACR Tasks and, by default, pushes the built image to its registry upon completion.
...
The first step in creating a Dockerfile is choosing a base image that serves as the foundation for your application. For example, if you're building a Python application, you might choose an official Python image as your base.
...
- `FROM python:3.9`: This command sets the base image to Python 3.9, which provides the Python runtime environment for your application.
- `WORKDIR /app`: Sets the working directory to `/app`, which is where app files will be copied.
- `COPY requirements.txt .`: Copies the requirements.txt file to the container's `/app` directory. This file lists the Python dependencies your application needs.
...
- `EXPOSE 8000`: Exposes port 8000, commonly used for Python web applications, to the outside world. Change this line if your app listens on a different port.
- `CMD ["python", "app.py"]`: The command to run when the container starts. In this case, we're running the python command with your application's entry point (`app.py`). Change this line to match your app's main file.

We're not going to cover the Dockerfile file specification. Visit the [Dockerfile reference](https://docs.docker.com/reference/dockerfile/) for more information. Each of these steps creates a cached container image as we build the final container image. These temporary images are layered on top of the previous and presented as single image once all steps complete.
...
Applications built on Azure Container Apps can dynamically scale based on: HTTP traffic, event-driven processing, CPU or memory load, and any [KEDA-supported scaler](https://keda.sh/docs/2.16/scalers/).
...
6. Create the resource group for your container app.
...
For more information about Container Apps commands, visit the [az containerapp](https://learn.microsoft.com/en-us/cli/azure/containerapp?view=azure-cli-latest) reference.
